...
{
    public partial class frmPrintReceiptBojo : Form
    {
        string _name, _current, _previous, _date, _bill, _consume, _lessCBM, _totalCBM, _amountPerCBM;

        public frmPrintReceiptBojo(string name, string current, string previous, string date, string bill, string consume, string lessCBM, string totalCBM, string amountPerCBM)
        {
            InitializeComponent();
            _name = name;
            _current = current;
        ...
        private void frmPrintReceiptBojo_Load(object sender, EventArgs e)
        {
            ...
            this.reportViewerPrintReceiptBojo.LocalReport.SetParameters(para);
            this.reportViewerPrintReceiptBojo.RefreshReport();
        }
    }
}
